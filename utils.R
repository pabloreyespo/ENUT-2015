...
get_data <- function(
  ...
  model_data <- haven::read_dta("data/enut-i-25G.dta") %>%
    filter(edad_años >= 18, es_trabajador == 1, w > 0, ing_personal > 0, total_expenses/ing_personal <= 5) %>%
    mutate(female = sexo,
...
get_data_tc <- function(especificas = c(), disputed = "t_sleep") {
  ...
  acts  <-c("t_personal_care", "t_sleep","t_meals", "t_commute1", "t_job_search", "t_care_work", "t_domestic_work", "t_unpaid_voluntary", "t_education")
