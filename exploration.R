...
dbs = model_data
data_stack <- dbs %>%
  dplyr::select(id_persona, edad_años, quintil, female, all_of(expenditures), all_of(times)) %>%
...
hist_tiempos <- data_stack %>%
  ggplot( aes(x=tiempo, fill = factor(tipo_tiempo, levels =c("Paid work", "Socializing", "Hobbies and sports", "Reading Media", "Listening Media", "Watching Media", "Computer, Phone", "Sleep", "Meals", "Commute", "Other")))) +
  ...
  facet_wrap(. ~ factor(tipo_tiempo, levels =c("Paid work", "Socializing", "Hobbies and sports","Reading Media", "Listening Media", "Watching Media", "Computer, Phone", "Sleep", "Meals", "Commute", "Other")),
             scales = "free", nrow = 3) +
  theme_tesis
hist_tiempos
...
hist_wage <- dbs %>%
  ...
  geom_histogram(alpha=0.8, position = 'identity', bins = 80) +
  ...
  geom_vline(xintercept = median(dbs$w), linetype="dashed", color = "red")+
  theme_tesis
hist_wage
...
hist_committed <- dbs %>% filter(ec > 0)%>%
...
#### INCOME VARIABLES
get_data()
original   <- c("edad_años","mayor45","menor25", "underage_in_household", "only_worker",
